<template>
	<div>
		...
		<hr class="my-3" />
		<Markdown id="math/math-tools/bottom" />
		<br />
		<ReturnToTop />
	</div>
</template>
...
export default {
	created() {
		this.$pageInfo(this, this.$t('common.tabs.math'), 'math');
	},
	destroyed() {
	...
	async asyncData({ $content, app }) {
		let toolsList = await $content(app.i18n.defaultLocale, 'math/math-tools/tools').fetch().catch(err => { return [] });
		if (app.i18n.defaultLocale !== app.i18n.locale) {
			try {
				const translatedPosts = await $content(app.i18n.locale, 'math/math-tools/tools').fetch();
